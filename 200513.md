...
#### 5.6 파일 전송 프로토콜(File Transfer Protocol, FTP)


파일 전송 프로토콜(File Transfer Protocol, FTP)은 TCP/IP 프로토콜을 가지고 서버와 클라이언트 사이의 파일 전송을 하기 위한 프로토콜이다. 유저에게 인증/식별/인가 과정을 거쳐 권한을 부여한다.


* FTP 정의
> * 클라이언트는 자신의 소스 포트로 1023보다 큰 임의의 번호를 사용하며, FTP 서버의 21번 포트를 사용
> * 파일 전송에 쓰이는 20번 포트가 쓰임
> * 능동 모드(Active Mode)
>   + 1. 클라이언트가 서버에 접근 요청
>   + 2. 서버가 클라이언트에게 허용 응답
>   + 3. 서버가 클라이언트의 포트에 연결 요청
>   + 4. 클라이언트가 서버에 허용 응답
> * 수동 모드(Passive Mode)
>   + 1. 클라이언트가 서버에 접근 요청
>   + 2. 서버가 클라이언트에게 허용 응답
>   + 3. 클라이언트가 서버의 포트에 연결 요청
>   + 4. 서버가 클라이언트에 허용 응답 
